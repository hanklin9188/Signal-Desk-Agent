...
function renderSettings() {
  ...
  $('#detailPane').innerHTML = `<div class="full-page">${specialHeader('PREFERENCES','設定','控制干擾、資料保留、模型常駐與本機隱私。')}<div class="settings-grid">
    ...
    <section class="settings-section"><h2>本機模型</h2><p>模型失效時，事件仍會保存並使用安全規則基線。</p><div class="setting-row"><div><strong>模型後端</strong><small>${esc(state.model.id || '')} · ${esc(state.model.quantization || 'none')}</small></div><span class="health-badge">${esc(state.model.backend || 'rule')}</span></div><div class="setting-row"><div><strong>GPU 駐留</strong><small>按需模式會在每批工作後釋放 VRAM</small></div><select data-setting="model_residency"><option value="on_demand" ${s.model_residency==='on_demand'?'selected':''}>On demand</option><option value="always_on" ${s.model_residency==='always_on'?'selected':''}>Always on</option><option value="auto_sleep" ${s.model_residency==='auto_sleep'?'selected':''}>Auto sleep</option><option value="paused" ${s.model_residency==='paused'?'selected':''}>Paused</option></select></div></section>
